...
export default function ActivoModal(props) {

    //Informacion necesaria para editar un grupo
    const [usuarios, setUsuarios] = useState([]);
    const [idUsuario, setIdUsuario] = useState(props.data._ast_usuarios);

    const [departamentos, setDepartamentos] = useState([]);
    const [idDepartamento, setIdDepartamento] = useState(props.data._ast_business_unit);
    ...
    const [code, setCode] = useState(props.data._code);
    const [name, setName] = useState(props.data._name);
    const [description, setDescription] = useState(props.data._description);
    const [ubicacion, setUbicacion] = useState(props.data._ubication);
    const [label, setLabel] = useState(props.data._label);
    const [type, setType] = useState(props.data._type);
    const [id_criterio, setId_criterio] = useState(props.data._id_criterio);
    const [id_categoria, setId_categoria] = useState(props.data._id_categoria);
    const [id_clasificacion, setId_clasificacion] = useState(props.data._id_clasificacion);
    const [date, setDate] = useState(props.data._review_date);
    ...
    const editar = () => {
        ...
        fecha.toISOString();
        putActivo(props.data._id, code, name, description, ubicacion, label, type, id_criterio, id_categoria, id_clasificacion, props.idOrganizacion, fecha).then((response) => {
        ...
        deleteActivoUsuario(props.data._id)
        deleteActivoBusinessUnitUsuario(props.data._id)
        usuariosActivo.map((usuario) => {
            postActivoUsuario(usuario._id, props.data._id)
        })
        departamentosActivo.map((departamento) => {
            postActivoBusinessUnitUsuario(props.data._id, departamento._id)
        })
    ...
    useEffect(() => {
        ...
        getUsuarios(props.idOrganizacion).then((response) => {
            const arrayUsuario = [];
            //Mapea los usuarios de props.data._ast_usuarios y compara con los usuarios de la organizacion, si existe  hace un push al state usuariosActivo
            props.data._ast_usuarios.map((usuario, index) => {
                console.log(usuario);
                const nuevoUsuario = response.find(user => user._id == usuario.usr_id_user);
                if (nuevoUsuario !== undefined && !usuariosActivo.includes(nuevoUsuario)) {
    ...
    useEffect(() => {
        ...
        getDepartamentos(props.idOrganizacion).then((response) => {
            ...
            //Mapea los departamentos de props.data._ast_business_unit y compara con los departamentos de la organizacion, si existe  hace un push al state departamentosActivo
            props.data._ast_business_unit.map((departamento, index) => {

                const nuevoDepartamento = response.find(unit => unit._id == departamento.bu_id_business_unit);
                if (nuevoDepartamento !== undefined && !departamentosActivo.includes(nuevoDepartamento)) {
                    arrayDepartamento.push(nuevoDepartamento);
                }
            })
                                    ...
                                    <div style={{ width: "50%" }}>
                                        ...
                                        >
                                            {departamentos?.map((departamento) => (
                                                <SelectItem key={departamento._id} value={idDepartamento._id}>
                                                    {departamento._nombre}
                                                </SelectItem>
                                            ))}
                                        </Select>
