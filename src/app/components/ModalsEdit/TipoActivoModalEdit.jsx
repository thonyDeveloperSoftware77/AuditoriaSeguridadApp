...
import { putTipoActivo } from "../../servicios/tipoActivoSrv";

export default function TipoActivoModal(props) {
    const [nombre, setNombre] = useState(props.data._nombre);
    const [descripcion, setDescripcion] = useState(props.data._descripcion);
    const { onOpen } = useDisclosure();

    const editar = () => {
        putTipoActivo(props.data._id, nombre, descripcion).then((response) => {
        ...
        <>
            ...
            <Modal backdrop={"blur"} isOpen={onOpen} >
                <ModalContent>
                    {(onClose) => (
                        <>
                            <ModalHeader className="flex flex-col gap-1">Actualia un Tipo de Activo</ModalHeader>
                            <ModalBody>
